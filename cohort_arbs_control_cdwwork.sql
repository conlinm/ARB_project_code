...
DECLARE @vocabulary_database_schema SYSNAME = 'OMOPV5';

DECLARE @cdm_database_schema SYSNAME = 'OMOPV5';
...
--cohort id 1: ARB users with no prior RCCa, CKD, transplant, or genetic risk, defined in cohort_arbs_cdwwork.sql
-- cohort id 0: control cohort defined in this script
-- cohort id 2: outcome cohort defined in cohort_arbs_outcome_cdwwork.sql 
...
INSERT INTO
    #Codesets (codeset_id, concept_id)
SELECT
    0 as codeset_id,
    c.concept_id
FROM
        ...
        FROM
            (
                ...
                from
                    @vocabulary_database_schema.CONCEPT
                where
                    concept_id in (45765451, 45773365, 37116954)
                UNION
                select
                    c.concept_id
                from
                    @vocabulary_database_schema.CONCEPT c
                    join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
                    and ca.ancestor_concept_id in (45765451, 45773365, 37116954)
                    and c.invalid_reason is null
            ) I
    ) C
UNION ALL
        ...
        FROM
            (
                ...
                from
                    @vocabulary_database_schema.CONCEPT c
                    join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
                    and ca.ancestor_concept_id in (4263213, 380839, 4110719)
                    and c.invalid_reason is null
            ) I
    ) C
UNION ALL
        ...
        FROM
            (
                ...
                from
                    @vocabulary_database_schema.CONCEPT
                where
                    concept_id in (37395652, 443611, 193782)
                UNION
                select
                    c.concept_id
                from
        ...
        FROM
            (
                ...
                from
                    @vocabulary_database_schema.CONCEPT
                where
                    concept_id in (21601783, 21601801, 21601744, 21601461, 21601665)
                UNION
                select
                    c.concept_id
                from
                    ...
                    (
                        ...
                        FROM
                            (
                                ...
                                WHERE
                                    DATEDIFF(d, C.start_date, C.end_date) > 60
                        ...
                        WHERE
                            DATEADD(day, 365, OP.OBSERVATION_PERIOD_START_DATE) <= E.START_DATE
                    ...
                    (
                        ...
                        FROM
                            ...
                            INNER JOIN (
                                ...
                                WHERE
                                    YEAR(E.start_date) - P.year_of_birth > 18
                                GROUP BY
                                    e.person_id,
                                    e.event_id
                                                    ...
                                                    (
                                                        SELECT
                                                            ...
                                                            COALESCE(
                                                                ...
                                                                DATEADD(day, 1, co.condition_start_date)
                                                            ) as end_date
                                                        FROM
                                                    ...
                                                    (
                                                        select
                                                            ...
                                                            COALESCE(
                                                                ...
                                                                DATEADD(day, de.days_supply, de.drug_exposure_start_date),
...
FROM
    (
        ...
        WHERE
            (
                MG.inclusion_rule_mask = POWER(cast(2 as bigint), 6) -1
            )
    ) Results
WHERE
    Results.ordinal = 1;
                ...
                from
                    ...
                    join ( -- cohort_ends
                                ...
                                from
                                    (
                                        select
                                            ...
                                            DATEADD(day, 1, d.death_date) as end_date
                                        FROM
                                            @cdm_database_schema.DEATH d
                                    ) C
                                    -- End Death Criteria
                            ) C on C.person_id = I.person_id
                            and C.start_date >= I.start_date
                        ...
                        GROUP BY
                            i.event_id,
                            i.person_id
                    ) CE on I.event_id = CE.event_id
                    and I.person_id = CE.person_id
    ...
    ( --cteEnds
        ...
        FROM
            ...
            JOIN ( -- cteEndDates
                ...
                WHERE
                    interval_status = 0
            ) ed ON c.person_id = ed.person_id
            AND ed.end_date >= c.start_date
        GROUP BY
            c.person_id,
            c.start_date
    ) e
